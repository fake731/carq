...
const AuthPage = () => {
  ...
  const handleRegister = (e: React.FormEvent) => {
    ...
    const creds = register(fullName, phone);
    if (creds) {
        ...
        {credentials && (
          <div className="glass-strong rounded-2xl p-8 text-center space-y-4">
            ...
            <button
              ...
              className="w-full py-3 rounded-xl bg-primary text-primary-foreground font-bold text-lg hover:opacity-90 transition-opacity neon-glow"
            >
              متابعة
            </button>
          </div>
            ...
            <div className="flex gap-1 mb-6 bg-surface-2 rounded-xl p-1">
              <button
                ...
                className={`flex-1 py-2.5 rounded-lg text-sm font-bold transition-all ${
                  mode === "login" ? "bg-primary text-primary-foreground neon-glow" : "text-muted-foreground hover:text-foreground"
                }`}
              >
                تسجيل الدخول
              </button>
              <button
                ...
                className={`flex-1 py-2.5 rounded-lg text-sm font-bold transition-all ${
                  mode === "register" ? "bg-primary text-primary-foreground neon-glow" : "text-muted-foreground hover:text-foreground"
                }`}
              >
                حساب جديد
              </button>
              ...
              <form onSubmit={handleLogin} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-surface-2 border border-border rounded-xl px-4 py-3 text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/30 transition-all"
                ...
                <p className="text-center text-xs text-muted-foreground mt-3">
                  جرّب: ahmed_h / أي كلمة مرور (عميل) | alnoor_garage (كراج) | bmw555 / 123456789 (أدمن)
                </p>
              </form>
            ) : (
